...
void manual_compute_join_indices(const std::shared_ptr<arrow::Array>& left_key,
    ...
    for (int64_t i = 0; i < left_typed->length(); ++i) {
        if (!left_typed->IsValid(i)) continue;
...
EagerDataFrame EagerDataFrame::with_column(const std::string& name, const Expr& expr) const {
    ...
    if (!result.ok()) {
        throw std::runtime_error("AddColumn Error: " + result.status().ToString());
    }
...
EagerDataFrame EagerDataFrame::join(const EagerDataFrame& other, const std::vector<std::string>& on, const std::string& how) const {
    ...
    for (int i = 0; i < table_->num_columns(); ++i) {
        ...
        switch (col_array->type_id()) {
            case arrow::Type::INT32: taken_arr = manual_take_join<arrow::Int32Type>(col_array, visitor.left_indices); break;
            case arrow::Type::INT64: taken_arr = manual_take_join<arrow::Int64Type>(col_array, visitor.left_indices); break;
            case arrow::Type::FLOAT: taken_arr = manual_take_join<arrow::FloatType>(col_array, visitor.left_indices); break;
            case arrow::Type::DOUBLE: taken_arr = manual_take_join<arrow::DoubleType>(col_array, visitor.left_indices); break;
            case arrow::Type::STRING: taken_arr = manual_take_join<arrow::StringType>(col_array, visitor.left_indices); break;
            case arrow::Type::BOOL: taken_arr = manual_take_join<arrow::BooleanType>(col_array, visitor.left_indices); break;
